...
%Use antigenic distance as denumerator
%Subplot1 f(k,V)
...
p = 1; %original value is 4
%r = 1;
r = 70;
b = 3;
a = 0.7;
c = 0.5; % contact rate
nv = 3; % average copies number of each virion
...
D = 0.55; %antigenic distance by boosting
delta = 0.2; %antigenic distance by drift
...
%for k=1:20:21;   
   ...
   if k>=1
    for  j=1:k
        temp = temp + (1/(D+delta*(k+1-j)))
    end
   end
   D_eff = 1/temp;
   P_Trans = (P_s0).^(1./D_eff);
