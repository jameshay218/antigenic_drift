...
if 1
    ...
    for i = 1:n_tot_samples
        ...
        % pick uniformly from the last 10% of the time interval-- TEMP!!!
        %locs = floor(0.9*n_time_intervals) + randi(floor(0.1*n_time_intervals));  % pick uniformly from time during epidemic
        
        %date_of_isolation = timeSeries.t_vals(locs(1));
        %date_of_isolation = epi_params.tRange_stoch(1) + (epi_params.tRange_stoch(2) - epi_params.tRange_stoch(1))*rand;
        % adjusted by tRange_min
        %date_of_isolation = epi_params.tRange_stoch(1) + epi_params.tRange_min + (epi_params.tRange_stoch(2) - epi_params.tRange_stoch(1))*rand;
        date_of_isolation = epi_params.tRange_stoch(1) + (epi_params.tRange_stoch(2) - epi_params.tRange_stoch(1))*rand;
