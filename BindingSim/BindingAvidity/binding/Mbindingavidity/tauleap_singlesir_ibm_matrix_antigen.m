...
VirusesArray = zeros(20E5,8); %what is the max capacity and max population size 20E6 too large
%VirusesArray=initVirues
%May 11, 2013 11E6
VirusesArray = initViruses;
%CurrentViruses = [];
...
dat=open(['dat/seed.mat']);
params.mi = dat.mi;
params.mu_list = dat.mu_list;
...
prvN_Infect = 100;
S = zeros(params.N_Infect,1);%%Create Susceptible groups from S0-S99
I = zeros(params.N_Infect,1);%%Create Infected groups from I1-I100
R = zeros(params.N_Infect,1);%%Create Recovered groups from R1-R100
S(1:prvN_Infect) = DataX(1,1:prvN_Infect)';   
I(1:prvN_Infect) = DataX(1,prvN_Infect+1:prvN_Infect*2)';  %20x5 I groups
R(1:prvN_Infect) = DataX(1,prvN_Infect*2+1:prvN_Infect*3)'; %20 R groups
...
if CurrTime>1 & rem(CurrTime,10) == 0
 disp(['Days ' num2str(CurrTime) ': ' num2str(vpopsize) ', ' num2str(maxvid)]);
...
%antigen.muprob = 0.01;
%antigen.mu = 10E-3;
%antigen.muratio = 250; %10 times larger than by product
%antigen.bypro = 0;
%if CurrTime > 1 %only mutation for day 1
%    antigen.muprob = 0.001;
%end
%default
antigen.muratio = 0;
antigen.muprob = 0;
...
model = 3;
%model0--No mutation, for testing only boosting
if model == 0
antigen.muratio = 0;
antigen.muprob = 0;
antigen.bypro = 0;
...
antigen.mu = 10E-2;
%model1--only binding avidity changes driven
if model == 1
antigen.muratio = 0;
antigen.muprob = 0;
antigen.bypro = 2;
...
%model3--antigenic changes with occasional large mutants
if model == 4
antigen.muratio = 12;
antigen.muprob = 0.01;
antigen.bypro = 10;
...
if CurrTime == 1
%   antigen.muprob = 0.005;
end
%

%mu = 10E-4; %20140512: 10E-4
%            %20140513: 10E-5 -> Viruses extinction
da = 1; % Mutations occur on 2nd day after virus birth
mu_id =find(CurrentViruses(:,VirusCol.birth)== CurrTime-da);
vidmu_list = CurrentViruses(mu_id,VirusCol.vid);
mu_list_tmp = exprnd(antigen.mu.*antigen.muratio,length(vidmu_list),1);
mi = find(rand(1,length(vidmu_list))<antigen.muprob);
mu_list = zeros(length(vidmu_list),1);
mu_list(mi) = mu_list_tmp(mi);
if CurrTime == -1
  mi = params.mi;
...
if CurrTime > 0
vinimu_list = CurrentViruses(mu_id,VirusCol.currentV);
end
CurrentViruses(mu_id,VirusCol.antigenicMU)=mu_list;
CurrentViruses(mu_id,VirusCol.antigenicTOT)=CurrentViruses(mu_id,VirusCol.antigenicTOT)+mu_list;

%obtain total_mu_list 
%getNewImmuneStatus( k, mu, p, v, r );
%VirusCol = struct('vid',1,'birth',2,'death',3,'parent',4,'infectionK',5,'beta',6,'initialV',7,'currentV',8,'antigenicMU',9,'antigenicBP',10,'antigenicTOT',11);
currentImmJ = round(k_list-VirusesArray(vid_list,VirusCol.antigenicTOT));
deltavrate_list = getDeltaV(vini_list, currentImmJ);    

if ~isempty(vidmu_list) %If there are 
    %antig_list = CurrentViruses(mu_id,VirusCol.antigenicTOT);
    %newk_list = getNewImmuneStatusLinear(CurrentViruses(mu_id,VirusCol.infectionK), antig_list);
    %[NoViruses x 100] for new K
    %newk_list2d = getNewImmuneStatusLinear2D( [0:params.N_Infect-1], antig_list); %[vid x immuneK]; used to generate beta matrix
    %replace k_list by newk_list
    %k_list_update = k_list-VirusesArray(vid_list,VirusCol.infectionK);
    %k_list_update(mu_id,1) = newk_list;
    %deltavrate_list = getDeltaV(vini_list, k_list_update); %update deltavrate for all current viruses
else
    %deltavrate_list = getDeltaV(vini_list, k_list-VirusesArray(vid_list,VirusCol.antigenicTOT));    
end
% update all current binding avidity based on updated immune status
deltav_list = deltavrate_list.*time_step;
vcurr_list = vini_list + deltav_list;

%%
%%add antigenic mutation effects as by-product
%%by-product changes every day
prop = antigen.bypro;
mu_bp_list = abs(deltav_list.*antigen.mu.*prop);
...
newk_list2d = getNewImmuneStatusLinear2D( [0:params.N_Infect-1], antig_list);
newk_list2d(find(newk_list2d<0))=0;
%%Need to change this
%[100 x NoViruses]
...
dRecoverBoost = zeros(params.N_Infect,1);
rm_id = find(R_indv==1);
k_list = CurrentViruses(rm_id, VirusCol.infectionK); %return a list of k
%return a current immune status K
k_uni_list = unique(k_list);
for i=1:length(k_uni_list)
    k = k_uni_list(i);
    novirus_k = length(k_list(k_list==k));
    rm_id_k = find(CurrentViruses(rm_id, VirusCol.infectionK)==k);
    k_list_adjust = CurrentViruses(rm_id(rm_id_k), VirusCol.antigenicTOT)- CurrentViruses(rm_id(rm_id_k), VirusCol.infectionK); %return a list of k using antigenicTOT
    k_list_adjust(find(k_list_adjust<0)) = 0;
    k_list_adjust = round(k_list_adjust);
    dRecover(k) = dRecover(k)+novirus_k;
    ab = poissrnd(params.AbB,novirus_k,1);
    for a=1:length(ab)
        if ab(a)+k> params.N_Infect
          ab(a) = params.N_Infect-k;
        end
        dRecoverBoost(k+ab(a)+k_list_adjust(a)) = dRecoverBoost(k+ab(a)+k_list_adjust(a)) + 1;
    end
end
if length(dRecover) ~= length(dRecoverBoost)
  disp('something is wrong calculating dRecoverBoost')
end
%%%Infection
...
R=R+dRecoverBoost;%8) New recovered individuals after antibody Boosting
...
ns_list = find(number_new_viruses>0); %the index of the viruses who produce offsprings
    ...
    for i =1:length(ns_list) %each source viruses     
        ...
        beta_j = zeros(1,params.N_Infect);         %transmission rate, temporarily set as 0
